...
const logDir = './log/cust_request_workflow_router_handler';
...
const logger = new (winston.Logger)({
  transports: [
	...
	new (require('winston-daily-rotate-file'))({
	  filename: `${logDir}/cust_request_workflow_router_handler_`,
...
function Cust_Request_WorkFlow_Router_Handler(){
				...
				//console.log(data);
					...
					var pickup = {
								...
								district: {
									...
									fee_surcharge_code:  (data[0].fee_surcharge_code ==null ? null : data[0].fee_surcharge_code)  
								},
								region: {
	...
	if(global.debug) console.log ('__filename: '+__filename+' & func: '+'Status_Request_Enquiry');
					...
					*/	
					if(rw_cache_id == null &&
						...
						)  
						{
							...
							logger.info(currentDate  + 'func: Status_Request_Enquiry | request_id= '+ jData.request_id + ' |  mobile_token='+ jData.mobile_token  +' | global.request_queue[i].request_id is FOUND');	
							...
							if(!global.request_queue[i].request.workflow.wf_closed_driver_not_available_datetime  ) {
								if(global.request_queue[i].request.workflow.wf_cust_assess_driver_datetime == null ) {
									...
									//global.request_queue[i].request.workflow.wf_cust_assess_driver_datetime   == null
											...
											var currentDate = moment(new Date()).format("YYYY-MM-DD H:mm:ss");
											logger.info(currentDate  + 'func: Status_Request_Enquiry | request_id= '+ jData.request_id + ' |  mobile_token='+ jData.mobile_token  + ' | rw_cache_id=' + rw_cache_id+' | rw_cache_id is TRUE');	
		...
		//if (! rw_cache_id) {		//serious bug because  the rw_cache_id may is 0 
		if (rw_cache_id == null ) {
			requestArray = null;
			var currentDate = moment(new Date()).format("YYYY-MM-DD H:mm:ss");
			logger.info(currentDate  + 'func: Status_Request_Enquiry | request_id= '+ jData.request_id + ' |  mobile_token='+ jData.mobile_token   + ' | rw_cache_id=' + rw_cache_id +' | ! rw_cache_id is TRUE | return NULL');	
			console.log('API: status > Cache NOT Found');
			} else {
					var currentDate = moment(new Date()).format("YYYY-MM-DD H:mm:ss");
					logger.info(currentDate  + 'func: Status_Request_Enquiry | request_id= '+ jData.request_id + ' |  mobile_token='+ jData.mobile_token   + ' | rw_cache_id=' + rw_cache_id +' | ! rw_cache_id is FASE | return OBJECT');	
					...
					//donation.status =  0;			
					donation.status =  (jData.donation_is == 1 ? true : false);
