...
/*
include new model
/model/OMS_Cust_Request_WorkFlow_Model
		...
		for (var i = 0; i < RW_MAX_CACHE; i++) {
		  if(global.request_queue[i]!=undefined  && global.request_queue[i]!=null) {
			...
			//logger.info('element['+i+'].request_id=' + global.request_queue[i].request_id);
				//setTimeout(this.Workflow_Start_Waterfall(i, function(data) {}), 10 );	//Unit: 10 msec 
				//setTimeout(this.Workflow_Start_Waterfall(i, function(data)) { console.log('something') }, 10); 
					...
					if (! global.request_queue[seq].request.workflow.wf_driver_matched_datetime ) {
						if (! global.request_queue[seq].request.nearly_driver.driver_user_id) {
							...
							//console.log('seq(08) is true');
								...
								logger.info('driver_handshaking_pool =' + driver_handshaking_pool);	
								logger.info('seq(08)  > Matching one driver nearby : element['+seq+'].request_id=' + global.request_queue[seq].request_id);								
								logger.info('seq(08)  > Matching one driver nearby : element['+seq+'] , filter_driver =' + filter_driver);								
								...
								if(Math.round((secondsDiff*10)/10) > 30 ) { // > 30 seconds
