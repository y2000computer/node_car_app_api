...
function Cust_Request_WorkFlow_Handler(){
	...
	var RW_DGDROP_GPS_LOST_TO = global.RW_DGDROP_GPS_LOST_TO	//Driver goto drop location GPS lost tolerance Unit: second
				...
				for (var i = 0; i < data.length; i++) {
					...
					var pickup = {
								...
								district: {
									...
									fee_surcharge_code:  (data[i].fee_surcharge_code ==null ? null : data[i].fee_surcharge_code)  
								},
								region: {
					...
					var nearly_driver = {
								create_datetime: data[i].nearly_driver_create_datetime, 
								driver_user_id: data[i].nearly_driver_user_id, 
								registration_id: data[i].nearly_driver_registration_id, 
								geo: {
									lat: data[i].nearly_driver_lat, 
									lng: data[i].nearly_driver_lng, 
								},
								nearly_driver_history: data[i].nearly_driver_history, 
								matching_timer: data[i].nearly_driver_matching_timer 
							};	
					...
					var gps_monitor = {
								driver_goto_drop_gps_timer: null 
							};								
	...
	//if(global.debug) console.log ('__filename: '+__filename+' & func: '+'Workflow_Start_Waterfall');
		...
		async.waterfall([
		  function(callback){
			...
			if(global.request_queue[seq]!=undefined  && global.request_queue[seq]!=null) {
			...
			if (! global.request_queue[seq].request.workflow.wf_closed_datetime) {
				if (global.request_queue[seq].request.workflow.wf_closed_driver_cancel_request_datetime ||
					...
					if (global.request_queue[seq].driver.id) {
						...
						var record = dmOMS.Driver_Online_Handshaking_Clear_Update(global.request_queue[seq].driver.id, function() {});
					...
					if(global.request_queue[seq].request.nearly_driver.driver_user_id) {
						...
						var record = dmOMS.Driver_Online_Handshaking_Clear_Update(global.request_queue[seq].request.nearly_driver.driver_user_id, function() {});
						if(index > -1) driver_handshaking_pool.splice(index, 1);
				...
				if (! global.request_queue[seq].request.workflow.wf_closed_driver_cancel_request_datetime &&
					...
					if (! global.request_queue[seq].request.workflow.wf_driver_matched_datetime) {
						...
						if(Math.round((secondsDiff*10)/10) > RW_RMDT ) {
							...
							var record = dmOMS.Close_And_Driver_Not_Available_Request_Update(global.request_queue[seq].request_id, function() {
						...
						if (! global.request_queue[seq].request.nearly_driver.driver_user_id) {
							...
							//console.log('seq(08) is true');
								...
								var jData = {
										...
										fee_surcharge_code:global.request_queue[seq].pickup.district.fee_surcharge_code
									};			
								...
								var record = handleDriverMatching.driver_matching(driver_handshaking_pool, seq, jData, function(data) {
									...
									if(data.driver_user_id) {
										...
										*/
										...
										var record = dmOMS.Match_Driver_Push_Message_Insert(jData, function() {});
										
										var jData = {
												request_id: global.request_queue[seq].request_id,
												create_datetime:moment(new Date()).format("YYYY-MM-DD H:mm:ss"),
												driver_user_id:  data.driver_user_id,
												registration_id:  data.registration_id,
												lat:  data.gps_latitude,
												lng:  data.gps_longitude,
												nearly_driver_history: global.request_queue[seq].request.nearly_driver.nearly_driver_history,
												matching_timer:moment(new Date()).format("YYYY-MM-DD H:mm:ss")
											};														
										var record = dmOMS.Request_Nearly_Driver_Update(jData, function() {});

										var record = dmOMS.Driver_Online_Handshaking_Update(data.driver_user_id, function() {});
										...
										var url = 'http://oms.hkber.com.hk/erp/en-US/middle_driver_push_message?request_id='+global.request_queue[seq].request_id+'driver_user_id='+data.driver_user_id;
		  ...
		  function(callback){
			//if(global.debug) console.log('execute seq(08a)');
			if(global.request_queue[seq]!=undefined  && global.request_queue[seq]!=null) {
			if (! global.request_queue[seq].request.workflow.wf_closed_datetime) {
				if (global.request_queue[seq].request.workflow.wf_driver_matched_datetime ) {
					//console.log('seq(08a) is true');
					if(global.request_queue[seq].request.nearly_driver.driver_user_id) {
						var record = dmOMS.Driver_Online_Handshaking_Clear_Update(global.request_queue[seq].request.nearly_driver.driver_user_id, function() {});
						var index = driver_handshaking_pool.indexOf(global.request_queue[seq].request.nearly_driver.driver_user_id);
						if(index > -1) driver_handshaking_pool.splice(index, 1);
				...
				if (! global.request_queue[seq].request.workflow.wf_closed_driver_cancel_request_datetime &&
					...
					if (! global.request_queue[seq].request.workflow.wf_driver_matched_datetime) {
				 	 if (global.request_queue[seq].request.nearly_driver.driver_user_id) {
						...
						var record = dmOMS.Driver_Online_Handshaking_Update(global.request_queue[seq].request.nearly_driver.driver_user_id, function() {});
						var index = driver_handshaking_pool.indexOf(global.request_queue[seq].request.nearly_driver.driver_user_id);
						if(index > -1) driver_handshaking_pool.splice(index, 1);
						...
						gps_distance  = gps_distance +(gps_distance*(30/100));  //Add 25 percents always
		  ...
		  function(callback){
			//if(global.debug) console.log('execute seq(15.2)');
			if(global.request_queue[seq]!=undefined  && global.request_queue[seq]!=null) {
			if (! global.request_queue[seq].request.workflow.wf_closed_datetime) {
				if (! global.request_queue[seq].request.workflow.wf_closed_driver_cancel_request_datetime &&
					! global.request_queue[seq].request.workflow.wf_closed_cust_cancel_request_datetime &&
					...
					if (global.request_queue[seq].request.workflow.wf_driver_matched_datetime &&
						...
						! global.request_queue[seq].request.workflow.wf_driver_arrive_drop_datetime ) {
						var ToCompare = null;
						if(global.request_queue[seq].request.gps_monitor.driver_goto_drop_gps_timer) ToCompare = global.request_queue[seq].request.gps_monitor.driver_goto_drop_gps_timer;
						if(!global.request_queue[seq].request.gps_monitor.driver_goto_drop_gps_timer) {
								ToCompare = moment(new Date()).format("YYYY-MM-DD H:mm:ss");
								global.request_queue[seq].request.gps_monitor.driver_goto_drop_gps_timer = ToCompare;
						}
						var startDate = moment(new Date()).format("YYYY-MM-DD H:mm:ss");
						//console.log('startDate=' +startDate);
						var endDate = moment(new Date(ToCompare)).format("YYYY-MM-DD H:mm:ss");
						...
						//console.log('secondsDiff=' +secondsDiff);
						if(Math.round((secondsDiff*10)/10) > 5 ) {
							//console.log('seq(15.2) is true');
							global.request_queue[seq].request.gps_monitor.driver_goto_drop_gps_timer = startDate;
							
							var record = dmOMS.Request_Last_GPS_Select(global.request_queue[seq].request_id, function(data) {
								var last_gps_server_log_datetime = null;
								if (data.length >0 ) {
										last_gps_server_log_datetime = data[0].server_log_datetime;
									} else {
										last_gps_server_log_datetime = moment(new Date()).format("YYYY-MM-DD H:mm:ss");
									}
									var endDate = last_gps_server_log_datetime;
									//console.log('endDate=' +endDate);
									var secondsDiff = moment(startDate).diff(endDate, 'seconds');
									//console.log('secondsDiff=' +secondsDiff);
									if(Math.round((secondsDiff*10)/10) > RW_DGDROP_GPS_LOST_TO ) {
										global.request_queue[seq].request.workflow.wf_driver_arrive_drop_datetime = last_gps_server_log_datetime;
										global.request_queue[seq].request.workflow.wf_closed_driver_drop_lost_gps_datetime = last_gps_server_log_datetime;
										global.request_queue[seq].request.status.code = "ERR_GOTO_DROP_LOST_GPS";
										var record = dmOMS.Driver_Arrive_Drop_GPS_Lost_Update(global.request_queue[seq].request_id,last_gps_server_log_datetime, function() {
												global.request_queue[seq].request.workflow.wf_closed_route_complete_datetime = last_gps_server_log_datetime;
												var jData = {
														request_id: global.request_queue[seq].request_id,
														driver_user_id:  global.request_queue[seq].driver.id
													};														
												var record = dmOMS.Driver_Online_Enabled_Update(jData, function() {
													logger.info('seq(15.2)  >  RW_DGDROP_GPS_LOST_TO had executed: element['+seq+'].request_id=' + global.request_queue[seq].request_id);
													});
											});
									}
							});					
													
							//logger.info('seq(15.2)  >  RW_DGDROP_GPS_LOST_TO : element['+seq+'].request_id=' + global.request_queue[seq].request_id);								
		  ...
		  function(callback){
			//if(global.debug) console.log('execute seq(20->15.5)');
			if(global.request_queue[seq]!=undefined  && global.request_queue[seq]!=null) {
			  if (! global.request_queue[seq].request.workflow.wf_closed_datetime) {
				if (! global.request_queue[seq].request.workflow.wf_closed_driver_cancel_request_datetime &&
					! global.request_queue[seq].request.workflow.wf_closed_cust_cancel_request_datetime &&
		  ...
		  function(callback){
			//if(global.debug) console.log('execute seq(16)');
				...
				if(global.request_queue[seq]!=undefined  && global.request_queue[seq]!=null) {
				if (global.request_queue[seq].request.workflow.wf_closed_datetime && global.request_queue[seq].request.workflow.wf_driver_arrive_drop_datetime &&
					  !global.request_queue[seq].request.workflow.wf_cust_assess_driver_datetime ) {
		  ...
		  function(callback){
			//if(global.debug) console.log('execute seq(16a)');
				//if (global.request_queue[seq].request_id != undefined) {
				if(global.request_queue[seq]!=undefined  && global.request_queue[seq]!=null) {
				if (global.request_queue[seq].request.workflow.wf_closed_datetime && global.request_queue[seq].request.workflow.wf_driver_arrive_drop_datetime && 
				       global.request_queue[seq].request.workflow.wf_cust_assess_driver_datetime ) {
					var ToCompare = global.request_queue[seq].request.workflow.wf_cust_assess_driver_datetime;
					var startDate = moment(new Date()).format("YYYY-MM-DD H:mm:ss");
					//console.log('startDate=' +startDate);
					var endDate = moment(new Date(ToCompare)).format("YYYY-MM-DD H:mm:ss");
					//console.log('endDate=' +endDate);
					var secondsDiff = moment(startDate).diff(endDate, 'seconds');
					//console.log('secondsDiff=' +secondsDiff);
					if(Math.round((secondsDiff*10)/10) > 30 ) {
						//console.log('seq(16a) is true');
						logger.info('driver_handshaking_pool =' + driver_handshaking_pool);	
						logger.info('seq(16a)  >  CLEAR QUEUE After 30 sec: element['+seq+'].request_id=' + global.request_queue[seq].request_id);								
		  ...
		  function(callback){
			//if(global.debug) console.log('execute seq(16b)');
				//if (global.request_queue[seq].request_id != undefined) {
				if(global.request_queue[seq]!=undefined  && global.request_queue[seq]!=null) {
				if (global.request_queue[seq].request.workflow.wf_closed_datetime && !global.request_queue[seq].request.workflow.wf_driver_arrive_drop_datetime ) {
					var ToCompare = null;
					if(global.request_queue[seq].request.workflow.wf_closed_driver_not_available_datetime) ToCompare = global.request_queue[seq].request.workflow.wf_closed_driver_not_available_datetime;
					if(global.request_queue[seq].request.workflow.wf_closed_driver_pickup_timeout_datetime) ToCompare = global.request_queue[seq].request.workflow.wf_closed_driver_pickup_timeout_datetime;
					if(global.request_queue[seq].request.workflow.wf_closed_driver_drop_timeout_datetime) ToCompare = global.request_queue[seq].request.workflow.wf_closed_driver_drop_timeout_datetime;
					if(global.request_queue[seq].request.workflow.wf_closed_driver_cancel_request_datetime) ToCompare = global.request_queue[seq].request.workflow.wf_closed_driver_cancel_request_datetime;
					if(global.request_queue[seq].request.workflow.wf_closed_cust_cancel_request_datetime) ToCompare = global.request_queue[seq].request.workflow.wf_closed_cust_cancel_request_datetime;
					if(global.request_queue[seq].request.workflow.wf_closed_oms_cancel_request_datetime) ToCompare = global.request_queue[seq].request.workflow.wf_closed_oms_cancel_request_datetime;
					...
					//console.log('secondsDiff=' +secondsDiff);
					if(Math.round((secondsDiff*10)/10) > 30 ) {
						//console.log('seq(16b) is true');
						logger.info('driver_handshaking_pool =' + driver_handshaking_pool);	
						logger.info('seq(16b)  >  CLEAR QUEUE After 30 sec: element['+seq+'].request_id=' + global.request_queue[seq].request_id);								
