...
function DmTestSection(){
	...
	this.conn.connect(function (err) {
			if(err) {
				...
				if (process.env.NODE_ENV == 'development') LogInfo.Info('func: ' + 'this.connect' + ' > no error ');
				}
	});
	...
	var query = this.conn.query('select * from tbl_test', function (err, results, fields) {
			if(err) {
				...
				if (process.env.NODE_ENV == 'development') LogInfo.Info('func: ' + 'this.list_all' + ' > no error ');
				fallback(results); }
				}
				);	
	...
	var query = this.conn.query('INSERT INTO tbl_test SET ?',JsonData,function (err, results ) {
			if(err) {
				...
				if (process.env.NODE_ENV == 'development') LogInfo.Info('func: ' + 'this.Insert' + ' > no error ');
	...
	this.Disconnect = function(fallback) {
		...
		this.conn.end(
			...
			if(err) {
				...
				//if (process.env.NODE_ENV == 'development') LogInfo.Info('func: ' + 'this.disconnect' + ' > no error ');
				}
			});
