...
function Test_Section_Model(){
	...
	this.conn.connect(function (err) {
			if(err) {
				if (process.env.NODE_ENV != 'production') LogErr.Err('func: ' + Model_Name + '.connect' + ' > SQL err: ' + err);
			} else {
	...
	if (process.env.NODE_ENV != 'production') console.log('execute List_All()');
	...
	if (process.env.NODE_ENV != 'production') console.log('execute Insert()');
	...
	this.Disconnect = function(fallback) {
		...
		this.conn.end(
			function(err){
			if(err) {
				 if (process.env.NODE_ENV != 'production') LogErr.Err('func: ' + Model_Name + '.Disconnect' + ' > SQL err: ' + err);
			} else {
