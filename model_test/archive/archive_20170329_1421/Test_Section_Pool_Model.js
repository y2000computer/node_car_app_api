...
const DB_Middle_Pool = require('../func/DB_Middle_Pool.js').DB_Middle_Pool;
...
function Test_Section_Pool_Model(){
	...
	this.Insert_Pool = function(JsonData,callback) {
    ...
    DB_OMS_Pool.getConnection(function(err,connection){
			...
			connection.query('INSERT INTO tbl_test SET ?',JsonData,function(err,results){
				connection.release();
				if(!err) {
	...
	this.List_All_DB_Middle_Pool = function(callback){
		
    DB_Middle_Pool.getConnection(function(err,connection){
			if (err) {
			  connection.release();
			  throw err;
			}   
