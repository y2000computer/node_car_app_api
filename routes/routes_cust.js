...
const Cust_Request_Location_History_Handler= require('../handler/Cust_Request_Location_History_Handler.js');	
...
router.route('/push_message_token')
    .put(function (req, res) {
        ...
        if (! j.push_message_token) { err.push('cust_reg_miss_push_message_token'); }
		...
		var record = dmOMS.Global_Token_Map_Select(j.token, function(token_data) {
				if (token_data.length == 0) { 
					...
					return; 
						...
						var jData = {
							token: j.token,
							push_message_token: j.push_message_token,
							create_datetime: Now,
							last_modify_datetime: Now
						};
						var dmOMS = new OMS_Global_Token_Model();
...
router.route('/request/donation')
	...
	router.route('/request/location_history')
    .post(function (req, res, next) {
		...
		var handle = new Cust_Request_Location_History_Handler();
		var record = handle.List(jData, function(data) {
				res.json(data);
			});
